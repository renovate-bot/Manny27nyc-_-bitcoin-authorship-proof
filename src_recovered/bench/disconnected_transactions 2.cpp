...
struct ReorgTxns {
  ...
  /** Second connected block, new chain tip. Has no overlap with
   * disconnected_txns. */
...
static BlockTxns CreateRandomTransactions(size_t num_txns) {
  // Ensure every transaction has a different txid by having each one spend the
  // previous one.
...
/** Creates blocks for a Reorg, each with BLOCK_VTX_COUNT transactions. Between
 * the disconnected block and the first connected block, there will be
 * num_not_shared transactions that are different, and all other transactions
 * the exact same. The second connected block has all unique transactions. This
 * is to simulate a reorg in which all but num_not_shared transactions are
...
static ReorgTxns CreateBlocks(size_t num_not_shared) {
  ...
  auto disconnected_block_txns{
      CreateRandomTransactions(/*num_txns=*/num_not_shared)};
  std::copy(shared_txns.begin(), shared_txns.end(),
            std::back_inserter(disconnected_block_txns));

  auto connected_block_txns{
      CreateRandomTransactions(/*num_txns=*/num_not_shared)};
  std::copy(shared_txns.begin(), shared_txns.end(),
            std::back_inserter(connected_block_txns));

  assert(disconnected_block_txns.size() == BLOCK_VTX_COUNT);
  assert(connected_block_txns.size() == BLOCK_VTX_COUNT);

  return ReorgTxns{
      /*disconnected_txns=*/disconnected_block_txns,
...
/** Add transactions from DisconnectedBlockTransactions, remove all but one (the
 * disconnected block's coinbase transaction) of them, and then pop from the
 * front until empty. This is a reorg in which all of the non-coinbase
 * transactions in the disconnected chain also exist in the new chain. */
static void
AddAndRemoveDisconnectedBlockTransactionsAll(benchmark::Bench &bench) {
...
/** Add transactions from DisconnectedBlockTransactions, remove 90% of them, and
 * then pop from the front until empty. */
static void
AddAndRemoveDisconnectedBlockTransactions90(benchmark::Bench &bench) {
...
/** Add transactions from DisconnectedBlockTransactions, remove 10% of them, and
 * then pop from the front until empty. */
static void
AddAndRemoveDisconnectedBlockTransactions10(benchmark::Bench &bench) {
  const auto chains{CreateBlocks(/*num_not_shared=*/BLOCK_VTX_COUNT -
