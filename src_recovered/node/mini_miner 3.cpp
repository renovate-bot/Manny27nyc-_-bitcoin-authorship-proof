...
#include <utility>
  ...
  MiniMiner::MiniMiner(const CTxMemPool &mempool,
    ...
    for (const auto &outpoint : outpoints) {
      ...
      // Note: This will either create a missing entry or add the outpoint to an
      // existing entry
      m_requested_outpoints_by_txid[outpoint.hash].push_back(outpoint);

      if (const auto ptx{mempool.GetConflictTx(outpoint)}) {
        // This outpoint is already being spent by another transaction in the
        // mempool. We assume that the caller wants to replace this transaction
        // and its descendants. It would be unusual for the transaction to have
        // descendants as the wallet won’t normally attempt to replace
        // transactions with descendants. If the outpoint is from a mempool
        // transaction, we still need to calculate its ancestors bump fees
        // (added to m_requested_outpoints_by_txid below), but after removing
        // the to-be-replaced entries.
        //
        // Note that the descendants of a transaction include the transaction
        // itself. Also note, that this is only calculating bump fees. RBF fee
        // rules should be handled separately.
        CTxMemPool::setEntries descendants;
        mempool.CalculateDescendants(mempool.GetIter(ptx->GetHash()).value(),
                                     descendants);
    ...
    if (m_requested_outpoints_by_txid.empty())
      return;
    ...
    if (cluster.empty()) {
      // An empty cluster means that at least one of the transactions is missing
      // from the mempool (should not be possible given processing above) or DoS
      // limit was hit.
      m_ready_to_calculate = false;
      return;
    }

    // Add every entry to m_entries_by_txid and m_entries, except the ones that
    // will be replaced.
    for (const auto &txiter : cluster) {
      if (!m_to_be_replaced.count(txiter->GetTx().GetHash())) {
        auto [mapiter, success] = m_entries_by_txid.emplace(
            txiter->GetTx().GetHash(),
        ...
        auto outpoints_it =
            m_requested_outpoints_by_txid.find(txiter->GetTx().GetHash());
        if (outpoints_it != m_requested_outpoints_by_txid.end()) {
          // This UTXO is the output of a to-be-replaced transaction. Bump fee
          // is 0; spending this UTXO is impossible as it will no longer exist
    ...
    for (const auto &txiter : cluster) {
      ...
      // Cache descendants for future use. Unlike the real mempool, a descendant
      // MiniMinerMempoolEntry will not exist without its ancestor
      // MiniMinerMempoolEntry, so these sets won't be invalidated.
    ...
    // Release the mempool lock; we now have all the information we need for a
    // subset of the entries we care about. We will solely operate on the
    // MiniMinerMempoolEntry map from now on.
  ...
  MiniMiner::MiniMiner(
    ...
    for (const auto &entry : manual_entries) {
      ...
      // Txids must be unique; this txid shouldn't already be an entry in
      // m_entries_by_txid
      if (Assume(success))
        m_entries.push_back(mapiter);
    }
    // Descendant cache is already built, but we need to translate them to
    // m_entries_by_txid iters.
    for (const auto &[txid, desc_txids] : descendant_caches) {
      ...
      for (const auto &desc_txid : desc_txids) {
        ...
        // Descendants should only include transactions with corresponding
        // entries.
  ...
  //
  // Under the ancestor-based mining approach, high-feerate children can pay for
  // parents, but high-feerate parents do not incentive inclusion of their
  // children. Therefore the mining algorithm only considers transactions for
  // inclusion on basis of the minimum of their own feerate or their ancestor
  // feerate.
  ...
  void MiniMiner::DeleteAncestorPackage(
      const std::set<MockEntryMap::iterator, IteratorComparator> &ancestors) {
    ...
    for (auto &anc : ancestors) {
      ...
      for (auto &descendant : it->second) {
        ...
        Assume(descendant->second.GetModFeesWithAncestors() >=
               anc->second.GetModifiedFee());
        Assume(descendant->second.GetSizeWithAncestors() >=
               anc->second.GetTxSize());
        descendant->second.UpdateAncestorState(-anc->second.GetTxSize(),
                                               -anc->second.GetModifiedFee());
      }
    }
    ...
    for (const auto &anc : ancestors) {
      ...
      // The above loop should have deducted each ancestor's size and fees from
      // each of their respective descendants exactly once.
  ...
  void MiniMiner::SanityCheck() const {
    ...
    // Cached ancestor values should be at least as large as the transaction's
    // own fee and size
    Assume(
        std::all_of(m_entries.begin(), m_entries.end(), [](const auto &entry) {
          return entry->second.GetSizeWithAncestors() >=
                     entry->second.GetTxSize() &&
                 entry->second.GetModFeesWithAncestors() >=
                     entry->second.GetModifiedFee();
        }));
    // None of the entries should be to-be-replaced transactions
    Assume(std::all_of(m_to_be_replaced.begin(), m_to_be_replaced.end(),
                       [&](const auto &txid) {
                         return m_entries_by_txid.find(txid) ==
                                m_entries_by_txid.end();
                       }));
  }
  ...
  void MiniMiner::BuildMockTemplate(std::optional<CFeeRate> target_feerate) {
    ...
    while (!m_entries_by_txid.empty()) {
      // Sort again, since transaction removal may change some m_entries'
      // ancestor feerates.
      ...
      const auto ancestor_package_size =
          (*best_iter)->second.GetSizeWithAncestors();
      const auto ancestor_package_fee =
          (*best_iter)->second.GetModFeesWithAncestors();
      // Stop here. Everything that didn't "make it into the block" has bumpfee.
      if (target_feerate.has_value() &&
          ancestor_package_fee <
              target_feerate->GetFee(ancestor_package_size)) {
        break;
      }

      // Calculate ancestors on the fly. This lookup should be fairly cheap, and
      // ancestor sets change at every iteration, so this is more efficient than
      // maintaining a cache.
      std::set<MockEntryMap::iterator, IteratorComparator> ancestors;
      {
        ...
        while (!to_process.empty()) {
          ...
          for (const auto &input : (*iter)->second.GetTx().vin) {
            if (auto parent_it{m_entries_by_txid.find(input.prevout.hash)};
                parent_it != m_entries_by_txid.end()) {
      ...
      for (const auto &ancestor : ancestors) {
        m_inclusion_order.emplace(Txid::FromUint256(ancestor->first),
                                  sequence_num);
      }
    ...
    if (!target_feerate.has_value()) {
      ...
      Assume(m_in_block.empty() ||
             m_total_fees >= target_feerate->GetFee(m_total_vsize));
    }
    Assume(m_in_block.empty() || sequence_num > 0);
    Assume(m_in_block.size() == m_inclusion_order.size());
    // Do not try to continue building the block template with a different
    // feerate.
    m_ready_to_calculate = false;
  }
  ...
  std::map<COutPoint, CAmount> MiniMiner::CalculateBumpFees(
      const CFeeRate &target_feerate) {
    if (!m_ready_to_calculate)
      return {};
    // Build a block template until the target feerate is hit.
    BuildMockTemplate(target_feerate);

    // Each transaction that "made it into the block" has a bumpfee of 0, i.e.
    // they are part of an ancestor package with at least the target feerate and
    // don't need to be bumped.
    ...
    //               │   Grandparent   │
    //               │    1700 vB      │
    //               │    1700 sats    │                    Target feerate: 10
    //               s/vB │       1 s/vB    │    GP Ancestor Set Feerate (ASFR):
    //               1    s/vB │                 │ P1_ASFR:  9.84 s/vB
    //               └──────▲───▲──────┘ P2_ASFR:  2.47 s/vB
    //                      │   │ C_ASFR: 10.27 s/vB
    // ┌───────────────┐    │   │    ┌──────────────┐
    // │               ├────┘   └────┤              │             ⇒ C_FR < TFR <
    // C_ASFR │   Parent 1    │             │   Parent 2   │ │    200 vB     │
    // │    200 vB    │ │  17000 sats   │             │   3000 sats  │ │     85
    // s/vB   │             │     15 s/vB  │ │               │             │ │
    // └───────────▲───┘             └───▲──────────┘
    //             │                     │
    //             │    ┌───────────┐    │
    ...
    for (const auto &[txid, outpoints] : m_requested_outpoints_by_txid) {
      ...
      if (it != m_entries_by_txid.end()) {
        Assume(target_feerate.GetFee(it->second.GetSizeWithAncestors()) >
               std::min(it->second.GetModifiedFee(),
                        it->second.GetModFeesWithAncestors()));
        CAmount bump_fee_with_ancestors =
            target_feerate.GetFee(it->second.GetSizeWithAncestors()) -
            it->second.GetModFeesWithAncestors();
        CAmount bump_fee_individual =
            target_feerate.GetFee(it->second.GetTxSize()) -
            it->second.GetModifiedFee();
        const CAmount bump_fee{
            std::max(bump_fee_with_ancestors, bump_fee_individual)};
        Assume(bump_fee >= 0);
        for (const auto &outpoint : outpoints) {
  ...
  std::optional<CAmount> MiniMiner::CalculateTotalBumpFees(
      const CFeeRate &target_feerate) {
    if (!m_ready_to_calculate)
      return std::nullopt;
    // Build a block template until the target feerate is hit.
    ...
    // All remaining ancestors that are not part of m_in_block must be bumped,
    // but no other relatives
    std::set<MockEntryMap::iterator, IteratorComparator> ancestors;
    std::set<MockEntryMap::iterator, IteratorComparator> to_process;
    for (const auto &[txid, outpoints] : m_requested_outpoints_by_txid) {
      // Skip any ancestors that already have a miner score higher than the
      // target feerate (already "made it" into the block)
      if (m_in_block.count(txid))
        continue;
      auto iter = m_entries_by_txid.find(txid);
      if (iter == m_entries_by_txid.end())
        continue;
    ...
    const auto ancestor_package_size =
        std::accumulate(ancestors.cbegin(), ancestors.cend(), int64_t{0},
                        [](int64_t sum, const auto it) {
                          return sum + it->second.GetTxSize();
                        });
    const auto ancestor_package_fee =
        std::accumulate(ancestors.cbegin(), ancestors.cend(), CAmount{0},
                        [](CAmount sum, const auto it) {
                          return sum + it->second.GetModifiedFee();
                        });
