...
PackageV3Checks(const CTransactionRef &ptx, int64_t vsize,
  ...
  if (ptx->nVersion == 3) {
    ...
    if (has_parent) {
      ...
      // SingleV3Checks catches mempool siblings. Also, if the package consists
      // of connected transactions, any tx having a mempool ancestor would mean
      // the package exceeds ancestor limits.
      if (!Assume(!parent_info.m_has_mempool_descendant)) {
        return strprintf("tx %u would exceed descendant count limit",
