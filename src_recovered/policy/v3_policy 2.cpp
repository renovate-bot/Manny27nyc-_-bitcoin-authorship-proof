...
#include <vector>
        ...
        /** Helper for PackageV3Checks: Returns a vector containing the indices
         * of transactions (within package) that are direct parents of ptx. */
            std::vector<size_t>
            FindInPackageParents(const Package &package,
...
struct ParentInfo {
  ...
  ParentInfo() = delete;
  ParentInfo(const Txid &txid, const Wtxid &wtxid,
             decltype(CTransaction::version) version,
...
PackageV3Checks(const CTransactionRef &ptx, int64_t vsize,
                const Package &package,
                const CTxMemPool::setEntries &mempool_ancestors) {
  // This function is specialized for these limits, and must be reimplemented if
  // they ever change.
  ...
  if (ptx->version == TRUC_VERSION) {
    ...
    const bool has_parent{mempool_ancestors.size() + in_package_parents.size() >
                          0};
    if (has_parent) {
      ...
      if (vsize > V3_CHILD_MAX_VSIZE) {
        return strprintf(
            ...
            ptx->GetHash().ToString(), ptx->GetWitnessHash().ToString(), vsize,
            V3_CHILD_MAX_VSIZE);
      }
      ...
      const auto parent_info = [&] {
        if (mempool_ancestors.size() > 0) {
          ...
          return ParentInfo{mempool_parent->GetTx().GetHash(),
                            ...
                            /*has_mempool_descendant=*/
                                mempool_parent->GetCountWithDescendants() > 1};
        } else {
      ...
      for (const auto &package_tx : package) {
        ...
        for (auto &input : package_tx->vin) {
          // Fail if we find another tx with the same parent. We don't check
          // whether the sibling is to-be-replaced (done in SingleV3Checks)
          // because these transactions are within the same package.
          ...
          if (input.prevout.hash == ptx->GetHash()) {
            return strprintf("tx %s (wtxid=%s) would have too many ancestors",
                             ...
                             package_tx->GetWitnessHash().ToString());
          }
        }
      }

      // It shouldn't be possible to have any mempool siblings at this point.
      // SingleV3Checks catches mempool siblings and sibling eviction is not
      // extended to packages. Also, if the package consists of connected
      // transactions, any tx having a mempool ancestor would mean the package
      // exceeds ancestor limits.
    ...
    for (auto it : mempool_ancestors) {
      if (it->GetTx().version == TRUC_VERSION) {
        return strprintf(
            ...
            ptx->GetHash().ToString(), ptx->GetWitnessHash().ToString(),
            it->GetSharedTx()->GetHash().ToString(),
            it->GetSharedTx()->GetWitnessHash().ToString());
      }
    }
...
SingleV3Checks(const CTransactionRef &ptx,
               const CTxMemPool::setEntries &mempool_ancestors,
               const std::set<Txid> &direct_conflicts, int64_t vsize) {
  // Check v3 and non-v3 inheritance.
  for (const auto &entry : mempool_ancestors) {
    if (ptx->version != TRUC_VERSION &&
        entry->GetTx().version == TRUC_VERSION) {
      return std::make_pair(
          strprintf(
              "non-v3 tx %s (wtxid=%s) cannot spend from v3 tx %s (wtxid=%s)",
              ptx->GetHash().ToString(), ptx->GetWitnessHash().ToString(),
              entry->GetSharedTx()->GetHash().ToString(),
              entry->GetSharedTx()->GetWitnessHash().ToString()),
          nullptr);
  ...
  if (vsize > V3_MAX_VSIZE) {
    return std::make_pair(
        strprintf("v3 tx %s (wtxid=%s) is too big: %u > %u virtual bytes",
                  ptx->GetHash().ToString(), ptx->GetWitnessHash().ToString(),
                  vsize, V3_MAX_VSIZE),
        nullptr);
  }
  ...
  if (mempool_ancestors.size() + 1 > V3_ANCESTOR_LIMIT) {
    return std::make_pair(
        strprintf("tx %s (wtxid=%s) would have too many ancestors",
                  ptx->GetHash().ToString(), ptx->GetWitnessHash().ToString()),
        nullptr);
  }
  ...
  if (mempool_ancestors.size() > 0) {
    ...
    // If there are any ancestors, this is the only child allowed. The parent
    // cannot have any other descendants. We handle the possibility of multiple
    // children as that case is possible through a reorg.
    const auto &children = parent_entry->GetMemPoolChildrenConst();
    // Don't double-count a transaction that is going to be replaced. This logic
    // assumes that any descendant of the V3 transaction is a direct child,
    // which makes sense because a V3 transaction can only have 1 descendant.
    const bool child_will_be_replaced =
        !children.empty() &&
        std::any_of(children.cbegin(), children.cend(),
                    [&direct_conflicts](const CTxMemPoolEntry &child) {
                      return direct_conflicts.count(child.GetTx().GetHash()) >
                             0;
                    });
    if (parent_entry->GetCountWithDescendants() + 1 > V3_DESCENDANT_LIMIT &&
        !child_will_be_replaced) {
      // Allow sibling eviction for v3 transaction: if another child already
      // exists, even if we don't conflict inputs with it, consider evicting it
      // under RBF rules. We rely on v3 rules only permitting 1 descendant, as
      // otherwise we would need to have logic for deciding which descendant to
      // evict. Skip if this isn't true, e.g. if the transaction has multiple
      // children or the sibling also has descendants due to a reorg.
      const bool consider_sibling_eviction{
          parent_entry->GetCountWithDescendants() == 2 &&
          children.begin()->get().GetCountWithAncestors() == 2};

      // Return the sibling if its eviction can be considered. Provide the
      // "descendant count limit" string either way, as the caller may decide
      // not to do sibling eviction.
      return std::make_pair(
          strprintf("tx %u (wtxid=%s) would exceed descendant count limit",
                    parent_entry->GetSharedTx()->GetHash().ToString(),
                    parent_entry->GetSharedTx()->GetWitnessHash().ToString()),
          consider_sibling_eviction ? children.begin()->get().GetSharedTx()
                                    : nullptr);
    }
  }
  return std::nullopt;
