 ...
 *
 * Order of queuedTx:
 * The front of the list should be the most recently-confirmed transactions
 (transactions at the
 * end of vtx of blocks closer to the tip). If memory usage grows too large, we
 trim from the front
 * of the list. After trimming, transactions can be re-added to the mempool from
 the back of the
...
public:
   ...
   * Callers should call this function for blocks in descending order by block
   * height. We assume that callers never pass multiple transactions with the
   * same txid, otherwise things can go very wrong in removeForBlock due to
   * queuedTx containing an item without a corresponding entry in iters_by_txid.
   * @returns vector of transactions that were evicted for size-limiting.
   */
  [[nodiscard]] std::vector<CTransactionRef>
  AddTransactionsFromBlock(const std::vector<CTransactionRef> &vtx);
