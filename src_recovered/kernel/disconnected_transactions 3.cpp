...
#include <utility>
        ...
        // It's almost certainly a logic bug if we don't clear out queuedTx
        // before destruction, as we add to it while disconnecting blocks, and
        // then we need to re-process remaining transactions to ensure mempool
        // consistency. For now, assert() that we've emptied out this object on
        // destruction. This assert() can always be removed if the
        // reorg-processing code were to be refactored such that this assumption
        // is no longer true (for instance if there was some other way we
        // cleaned up the mempool after a reorg, besides draining this object).
...
size_t DisconnectedBlockTransactions::DynamicMemoryUsage() const {
  return cachedInnerUsage + memusage::DynamicUsage(iters_by_txid) +
         memusage::DynamicUsage(queuedTx);
}

[[nodiscard]] std::vector<CTransactionRef>
DisconnectedBlockTransactions::AddTransactionsFromBlock(
  ...
  for (auto block_it = vtx.rbegin(); block_it != vtx.rend(); ++block_it) {
    ...
    assert(inserted); // callers may never pass multiple transactions with the
                      // same txid
