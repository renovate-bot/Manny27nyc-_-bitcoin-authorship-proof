...
#include <utility>
     ...
     *
     * PoolResource is not thread-safe. It is intended to be used by
     * PoolAllocator.
     *
     ...
     *
     * An example: If you create a PoolResource<128, 8>(262144) and perform a
     * bunch of allocations and deallocate 2 blocks with size 8 bytes, and 3
     * blocks with size 16, the members will look like this:
     *
     ...
     * m_free_lists[2] holds the 3 blocks of size 16. The blocks came from the
     * data stored in the m_allocated_chunks list. Each chunk has bytes 262144.
     * The last chunk has still some memory available for the blocks, and when
     * m_available_memory_it is at the end, a new chunk will be allocated and
     * added to the list.
     */
   ...
   * `m_allocated_chunks.back() + m_chunk_size_bytes` whenever it is accessed,
   * but `m_available_memory_end` caches this for clarity and efficiency.
   */
  std::byte *m_available_memory_end = nullptr;

  /**
   * How many multiple of ELEM_ALIGN_BYTES are necessary to fit bytes. We use
   * that result directly as an index into m_free_lists. Round up for the
   * special case when bytes==0.
   */
   ...
   */
  [[nodiscard]] static constexpr bool IsFreeListUsable(std::size_t bytes,
   ...
   *
   * Precondition: leftover bytes are either 0 or few enough to fit into a place
   * in the freelist
   */
  void AllocateChunk() {
    ...
    void *storage =
        ::operator new(m_chunk_size_bytes, std::align_val_t{ELEM_ALIGN_BYTES});
...
public:
  ...
  void *Allocate(std::size_t bytes, std::size_t alignment) {
    if (IsFreeListUsable(bytes, alignment)) {
      ...
      if (nullptr != m_free_lists[num_alignments]) {
        // we've already got data in the pool's freelist, unlink one element and
        // return the pointer to the unlinked memory. Since FreeList is
        // trivially destructible we can just treat it as uninitialized memory.
      ...
      // Make sure we use the right amount of bytes for that freelist (might be
      // rounded up),
  ...
  /**
   * Returns a block to the freelists, or deletes the block when it did not come
   * from the chunks.
   */
  void Deallocate(void *p, std::size_t bytes, std::size_t alignment) noexcept {
    if (IsFreeListUsable(bytes, alignment)) {
      const std::size_t num_alignments = NumElemAlignBytes(bytes);
      // put the memory block into the linked list. We can placement construct
      // the FreeList into the memory since we can be sure the alignment is
      // correct.
...
public:
  ...
  template <class U>
  PoolAllocator(
      const PoolAllocator<U, MAX_BLOCK_SIZE_BYTES, ALIGN_BYTES> &other) noexcept
      : m_resource(other.resource()) {}

  /**
   * The rebind struct here is mandatory because we use non type template
   * arguments for PoolAllocator. See
