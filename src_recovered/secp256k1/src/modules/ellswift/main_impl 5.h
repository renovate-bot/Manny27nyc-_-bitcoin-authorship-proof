...
#include "../../hash.h"
            ...
            SECP256K1_FE_CONST(0x851695d4, 0x9a83f8ef, 0x919bb861, 0x53cbcb16,
                               0x630fb68a, 0xed0a766a, 0x3ec693d6, 0x8e6afa40);
/** c2 = (-sqrt(-3)-1)/2 = -(c1+1) */
static const secp256k1_fe secp256k1_ellswift_c2 =
    SECP256K1_FE_CONST(0x7ae96a2b, 0x657c0710, 0x6e64479e, 0xac3434e9,
...
/** Decode ElligatorSwift encoding (u, t) to a fraction xn/xd representing a
 * curve X coordinate. */
static void secp256k1_ellswift_xswiftec_frac_var(secp256k1_fe *xn,
                                                 secp256k1_fe *xd,
   ...
   *
   * - Let c0 = sqrt(-3) =
   0xa2d2ba93507f1df233770c2a797962cc61f6d15da14ecd47d8d27ae1cd5f852.
   * - If u = 0, set u = 1.
   * - If t = 0, set t = 1.
   ...
   * - Return x1 = (X/Y-u)/2 (which is now guaranteed to be a valid x
   coordinate).
   *
  ...
  u1 = *u;
  if (EXPECT(secp256k1_fe_normalizes_to_zero_var(&u1), 0))
    u1 = secp256k1_fe_one;
  secp256k1_fe_sqr(&s, t);
  if (EXPECT(secp256k1_fe_normalizes_to_zero_var(t), 0))
    s = secp256k1_fe_one;
...
static void secp256k1_ellswift_xswiftec_var(secp256k1_fe *x,
                                            const secp256k1_fe *u,
                                            const secp256k1_fe *t) {
  secp256k1_fe xn, xd;
...
static void secp256k1_ellswift_swiftec_var(secp256k1_ge *p,
                                           const secp256k1_fe *u,
                                           const secp256k1_fe *t) {
  secp256k1_fe x;
...
/* Try to complete an ElligatorSwift encoding (u, t) for X coordinate x, given u
 * and x.
 *
 * There may be up to 8 distinct t values such that (u, t) decodes back to x,
 * but also fewer, or none at all. Each such partial inverse can be accessed
 * individually using a distinct input argument c (in range 0-7), and some or
 * all of these may return failure. The following guarantees exist:
 * - Given (x, u), no two distinct c values give the same successful result t.
 * - Every successful result maps back to x through
 * secp256k1_ellswift_xswiftec_var.
 * - Given (x, u), all t values that map back to x can be reached by combining
 * the successful results from this function over all c values, with the
 * exception of:
 ...
 *
 * The rather unusual encoding of bits in c (a large "if" based on the middle
 * bit, and then using the low and high bits to pick signs of square roots) is
 * to match the paper's encoding more closely: c=0 through c=3 match
 * branches 1..4 in the paper, while c=4 through c=7 are copies of those with an
 * additional negation of sqrt(w).
 */
static int secp256k1_ellswift_xswiftec_inv_var(secp256k1_fe *t,
                                               const secp256k1_fe *x_in,
                                               const secp256k1_fe *u_in,
                                               int c) {
  /* The implemented algorithm is this (all arithmetic, except involving c, is
   * mod p):
   *
   * - If (c & 2) = 0:
  ...
  if (!(c & 2)) {
    /* c is in {0, 1, 4, 5}. In this case we look for an inverse under the x1
     * (if c=0 or c=4) formula, or x2 (if c=1 or c=5) formula. */

    /* If -u-x is a valid X coordinate, fail. This would yield an encoding that
     * roundtrips back under the x3 formula instead (which has priority over x1
     * and x2, so the decoding would not match x). */
    m = x;                          /* m = x */
    secp256k1_fe_add(&m, &u);       /* m = u+x */
    secp256k1_fe_negate(&m, &m, 2); /* m = -u-x */
    /* Test if (-u-x) is a valid X coordinate. If so, fail. */
    if (secp256k1_ge_x_on_curve_var(&m))
      return 0;
    ...
    /* If s is not square, fail. We have not fully computed s yet, but s is
     * square iff
     * -(u^3+7)*(u^2+u*x+x^2) is square (because a/b is square iff a*b is square
     * and b is nonzero). */
    secp256k1_fe_sqr(&g, &u);              /* g = u^2 */
    secp256k1_fe_mul(&g, &g, &u);          /* g = u^3 */
    secp256k1_fe_add_int(&g, SECP256K1_B); /* g = u^3+7 */
    secp256k1_fe_mul(&m, &s, &g); /* m = -(u^3 + 7)*(u^2 + u*x + x^2) */
    if (!secp256k1_fe_is_square_var(&m))
      return 0;
    ...
    /* c is in {2, 3, 6, 7}. In this case we look for an inverse under the x3
     * formula. */
    ...
    /* If s is not square, fail. */
    if (!secp256k1_fe_is_square_var(&s))
      return 0;
    ...
    if (!secp256k1_fe_is_square_var(&q))
      return 0;
    ...
    /* If (c & 1) = 1 and r = 0, fail. */
    if (EXPECT((c & 1) && secp256k1_fe_normalizes_to_zero_var(&r), 0))
      return 0;

    /* If s = 0, fail. */
    if (EXPECT(secp256k1_fe_normalizes_to_zero_var(&s), 0))
      return 0;
  ...
  secp256k1_fe_mul(&u, &u,
                   c & 1 ? &secp256k1_ellswift_c4 : &secp256k1_ellswift_c3);
 ...
 *
 * hasher is a SHA256 object to which an incrementing 4-byte counter is written
 * to generate randomness. Writing 13 bytes (4 bytes for counter, plus 9 bytes
 * for the SHA256 padding) cannot cross a 64-byte block size boundary (to make
 * sure it only triggers a single SHA256 compression). */
static void secp256k1_ellswift_prng(unsigned char *out32,
  ...
  /* Writing and finalizing together should trigger exactly one SHA256
   * compression. */
  VERIFY_CHECK(((hash.bytes) >> 6) == (blocks + 1));
}

/** Find an ElligatorSwift encoding (u, t) for X coordinate x, and random Y
 * coordinate.
 *
 * u32 is the 32-byte big endian encoding of u; t is the output field element t
 * that still needs encoding.
 *
 * hasher is a hasher in the secp256k1_ellswift_prng sense, with the same
 * restrictions. */
static void
secp256k1_ellswift_xelligatorswift_var(unsigned char *u32, secp256k1_fe *t,
  ...
  int branches_left = 0;
  /* Field elements u and branch values are extracted from RNG based on hasher
   * for consecutive values of cnt. cnt==0 is first used to populate a pool of
   * 64 4-bit branch values. The 64 cnt values that follow are used to generate
   * field elements u. cnt==65 (and multiples thereof) are used to repopulate
   * the pool and start over, if that were ever necessary. On average, 4
  ...
  while (1) {
    ...
    --branches_left;
    branch =
        (branch_hash[branches_left >> 1] >> ((branches_left & 1) << 2)) & 7;
    ...
    /* Since u is the output of a hash, it should practically never be 0. We
     * could apply the u=0 to u=1 correction here too to deal with that case
     * still, but it's such a low probability event that we do not bother. */
    VERIFY_CHECK(!secp256k1_fe_normalizes_to_zero_var(&u));

    /* Find a remainder t, and return it if found. */
    if (EXPECT(secp256k1_ellswift_xswiftec_inv_var(t, x, &u, branch), 0))
      break;
  }
}
 ...
 *
 * This is similar secp256k1_ellswift_xelligatorswift_var, except it takes a
 * full group element p as input, and returns an encoding that matches the
 * provided Y coordinate rather than a random one.
 */
static void
secp256k1_ellswift_elligatorswift_var(unsigned char *u32, secp256k1_fe *t,
...
/** Set hash state to the BIP340 tagged hash midstate for
...
int secp256k1_ellswift_encode(const secp256k1_context *ctx,
  ...
  if (secp256k1_pubkey_load(ctx, &p, pubkey)) {
    ...
    /* Set up hasher state; the used RNG is H(pubkey || " "*31 || rnd32 ||
     * cnt++), using BIP340 tagged hash with tag "secp256k1_ellswift_encode". */
    ...
    secp256k1_ellswift_elligatorswift_var(ell64, &t, &p,
                                          &hash); /* puts u in ell64[0..32] */
...
int secp256k1_ellswift_create(const secp256k1_context *ctx,
  ...
  ret = secp256k1_ec_pubkey_create_helper(&ctx->ecmult_gen_ctx, &seckey_scalar,
                                          &p, seckey32);
  secp256k1_declassify(ctx, &p,
                       sizeof(p)); /* not constant time in produced pubkey */
  secp256k1_fe_normalize_var(&p.x);
  secp256k1_fe_normalize_var(&p.y);

  /* Set up hasher state. The used RNG is H(privkey || " "*32 [|| auxrnd32] ||
   * cnt++), using BIP340 tagged hash with tag "secp256k1_ellswift_create". */
  ...
  secp256k1_declassify(ctx, &hash,
                       sizeof(hash)); /* private key is hashed now */
  if (auxrnd32)
    secp256k1_sha256_write(&hash, auxrnd32, 32);
...
static int ellswift_xdh_hash_function_prefix(unsigned char *output,
...
static int ellswift_xdh_hash_function_bip324(unsigned char *output,
                                             const unsigned char *x32,
                                             const unsigned char *ell_a64,
                                             const unsigned char *ell_b64,
                                             void *data) {
...
int secp256k1_ellswift_xdh(const secp256k1_context *ctx, unsigned char *output,
                           const unsigned char *ell_a64,
                           const unsigned char *ell_b64,
                           const unsigned char *seckey32, int party,
