...
< ? php
         ...
         */
            *
            Copyright(c) 2022 Pieter Wuille **Distributed under the MIT software
    license,
 ...
 *     This is just a compile-time macro. Use the next macro to check it is
 ...
 *   - Assert or otherwise fail in case the len-byte memory block pointed to by
 * p is not considered entirely defined.
 ...
 *   - marks the len-byte memory block pointed to by p as undefined data (secret
 * data, in the context of constant-time checking).
 * - SECP256K1_CHECKMEM_DEFINE(p, len):
 *   - marks the len-byte memory pointed to by p as defined data (public data,
...
/* If valgrind integration is desired (through the VALGRIND define), implement
 * the SECP256K1_CHECKMEM_* macros using valgrind. */
