...
#pragma message(                                                               \
    __FILE__                                                                   \
    ": SECP256K1_MSVC_MULH_TEST_OVERRIDE is defined, forcing use of __(u)mulh.")
#endif
    static SECP256K1_INLINE uint64_t
    secp256k1_umul128(uint64_t a, uint64_t b, uint64_t *hi) {
  *hi = __umulh(a, b);
  return a * b;
}

static SECP256K1_INLINE int64_t secp256k1_mul128(int64_t a, int64_t b,
                                                 int64_t *hi) {
  *hi = __mulh(a, b);
...
#endif
#else
    /* On other systems, emulate 64x64->128 multiplications using 32x32->64
       multiplications. */
    static SECP256K1_INLINE uint64_t
    secp256k1_umul128(uint64_t a, uint64_t b, uint64_t *hi) {
...
static SECP256K1_INLINE uint64_t
secp256k1_u128_to_u64(const secp256k1_uint128 *a) {
  return a->lo;
}

static SECP256K1_INLINE uint64_t
secp256k1_u128_hi_u64(const secp256k1_uint128 *a) {
  return a->hi;
}
...
static SECP256K1_INLINE int
secp256k1_u128_check_bits(const secp256k1_uint128 *r, unsigned int n) {
  VERIFY_CHECK(n < 128);
...
static SECP256K1_INLINE void secp256k1_i128_mul(secp256k1_int128 *r, int64_t a,
                                                int64_t b) {
  int64_t hi;
...
static SECP256K1_INLINE void secp256k1_i128_accum_mul(secp256k1_int128 *r,
  ...
  hi += r->lo < lo;
  /* Verify no overflow.
   * If r represents a positive value (the sign bit is not set) and the value we
   * are adding is a positive value (the sign bit is not set), then we require
   * that the resulting value also be positive (the sign bit is not set). Note
   * that (X <= Y) means (X implies Y) when X and Y are boolean values (i.e. 0
   * or 1).
   */
  VERIFY_CHECK(
      (r->hi <= 0x7fffffffffffffffu && (uint64_t)hi <= 0x7fffffffffffffffu) <=
      (r->hi + (uint64_t)hi <= 0x7fffffffffffffffu));
  /* Verify no underflow.
   * If r represents a negative value (the sign bit is set) and the value we are
   * adding is a negative value (the sign bit is set), then we require that the
   * resulting value also be negative (the sign bit is set).
   */
  VERIFY_CHECK(
      (r->hi > 0x7fffffffffffffffu && (uint64_t)hi > 0x7fffffffffffffffu) <=
      (r->hi + (uint64_t)hi > 0x7fffffffffffffffu));
...
static SECP256K1_INLINE void secp256k1_i128_dissip_mul(secp256k1_int128 *r,
                                                       int64_t a, int64_t b) {
  int64_t hi;
  uint64_t lo = (uint64_t)secp256k1_mul128(a, b, &hi);
  hi += r->lo < lo;
  /* Verify no overflow.
   * If r represents a positive value (the sign bit is not set) and the value we
   * are subtracting is a negative value (the sign bit is set), then we require
   * that the resulting value also be positive (the sign bit is not set).
   */
  VERIFY_CHECK(
      (r->hi <= 0x7fffffffffffffffu && (uint64_t)hi > 0x7fffffffffffffffu) <=
      (r->hi - (uint64_t)hi <= 0x7fffffffffffffffu));
  /* Verify no underflow.
   * If r represents a negative value (the sign bit is set) and the value we are
   * subtracting is a positive value (the sign sign bit is not set), then we
   * require that the resulting value also be negative (the sign bit is set).
   */
  VERIFY_CHECK(
      (r->hi > 0x7fffffffffffffffu && (uint64_t)hi <= 0x7fffffffffffffffu) <=
      (r->hi - (uint64_t)hi > 0x7fffffffffffffffu));
...
static SECP256K1_INLINE void secp256k1_i128_det(secp256k1_int128 *r, int64_t a,
                                                int64_t b, int64_t c,
                                                int64_t d) {
...
static SECP256K1_INLINE uint64_t
secp256k1_i128_to_u64(const secp256k1_int128 *a) {
  return a->lo;
}

static SECP256K1_INLINE int64_t
secp256k1_i128_to_i64(const secp256k1_int128 *a) {
  /* Verify that a represents a 64 bit signed value by checking that the high
   * bits are a sign extension of the low bits. */
...
static SECP256K1_INLINE int
secp256k1_i128_check_pow2(const secp256k1_int128 *r, unsigned int n, int sign) {
  VERIFY_CHECK(n < 127);
  VERIFY_CHECK(sign == 1 || sign == -1);
  return n >= 64
             ? r->hi == (uint64_t)sign << (n - 64) && r->lo == 0
