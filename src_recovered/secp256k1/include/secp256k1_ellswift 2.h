...
#include "secp256k1.h"
   ...
   * f(u,t):
   * - Let C =
   * 0xa2d2ba93507f1df233770c2a797962cc61f6d15da14ecd47d8d27ae1cd5f852, a square
   * root of -3.
   ...
   * - Let X = (u^3 + 7 - t^2) / (2 * t)
   * - Let Y = (X + t) / (C * u)
   * - Return the first in [u + 4 * Y^2, (-X/Y - u) / 2, (X/Y - u) / 2] that is
   * an X coordinate on the curve (at least one of them is, for any u and t).
   *
  ...
  /** A pointer to a function used by secp256k1_ellswift_xdh to hash the shared
  ...
  typedef int (*secp256k1_ellswift_xdh_hash_function)(
      unsigned char *output, const unsigned char *x32,
   ...
   *  SHA256(prefix64 || ell_a64 || ell_b64 || x32), where prefix64 is the
   * 64-byte array pointed to by data. */
  SECP256K1_API const secp256k1_ellswift_xdh_hash_function
      secp256k1_ellswift_xdh_hash_function_prefix;

  /** An implementation of an secp256k1_ellswift_xdh_hash_function compatible
   * with BIP324. It returns H_tag(ell_a64 || ell_b64 || x32), where H_tag is
   * the BIP340 tagged hash function with tag "bip324_ellswift_xonly_ecdh".
   * Equivalent to secp256k1_ellswift_xdh_hash_function_prefix with prefix64 set
   * to
  ...
  SECP256K1_API int secp256k1_ellswift_encode(
      const secp256k1_context *ctx, unsigned char *ell64,
   ...
   *  Args:    ctx:        pointer to a context object
   *  Out:     ell64:      pointer to a 64-byte array to receive the
   * ElligatorSwift public key In:      seckey32:   pointer to a 32-byte secret
   * key auxrnd32:   (optional) pointer to 32 bytes of randomness
   *
   ...
   * it is optional (and does result in encodings that are indistinguishable
   * from uniform even without any auxrnd32). It differs from the (mandatory)
   * rnd32 argument to secp256k1_ellswift_encode in this regard.
   *
  ...
  SECP256K1_API SECP256K1_WARN_UNUSED_RESULT int secp256k1_ellswift_create(
      const secp256k1_context *ctx, unsigned char *ell64,
      const unsigned char *seckey32, const unsigned char *auxrnd32)
      SECP256K1_ARG_NONNULL(1) SECP256K1_ARG_NONNULL(2)
          SECP256K1_ARG_NONNULL(3);

  /** Given a private key, and ElligatorSwift public keys sent in both
   * directions, compute a shared secret using x-only Elliptic Curve
   * Diffie-Hellman (ECDH).
   *
   ...
   *
   * This function is more efficient than decoding the public keys, and
   * performing ECDH on them.
   */
  SECP256K1_API SECP256K1_WARN_UNUSED_RESULT int secp256k1_ellswift_xdh(
      const secp256k1_context *ctx, unsigned char *output,
