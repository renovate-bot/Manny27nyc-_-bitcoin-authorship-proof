  ...
  while (std::getline(stream, str)) {
    ...
    if (!str.empty()) {
      if (*str.begin() == '[' && *str.rbegin() == ']') {
        ...
        error =
            strprintf("parse error on line %i: %s, options in configuration "
                      "file must be specified without leading -",
                      linenr, str);
        return false;
      } else if ((pos = str.find('=')) != std::string::npos) {
        std::string name =
            prefix + TrimString(std::string_view{str}.substr(0, pos), pattern);
        std::string_view value =
            TrimStringView(std::string_view{str}.substr(pos + 1), pattern);
        if (used_hash && name.find("rpcpassword") != std::string::npos) {
          error = strprintf("parse error on line %i, using # in rpcpassword "
                            "can be ambiguous and should be avoided",
                            linenr);
          return false;
        }
...
bool IsConfSupported(KeyInfo &key, std::string &error) {
  if (key.name == "conf") {
    error = "conf cannot be set in the configuration file; use includeconf= if "
            "you want to include additional config files";
    return false;
  }
  if (key.name == "reindex") {
    // reindex can be set in a config file but it is strongly discouraged as
    // this will cause the node to reindex on every restart. Allow the config
    // but throw a warning
...
bool ArgsManager::ReadConfigFiles(std::string &error,
                                  bool ignore_invalid_keys) {
  {
    ...
    m_config_path =
        AbsPathForConfigVal(*this, GetPathArg("-conf", BITCOIN_CONF_FILENAME),
                            /*net_specific=*/false);
  }
  ...
  if (IsArgSet("-conf") && !stream.good()) {
    error = strprintf("specified config file \"%s\" could not be opened.",
  ...
  if (stream.good()) {
    ...
    // as `-noincludeconf` (which indicates that no included conf file should be
    // used).
    bool use_conf_file{true};
    {
      ...
      if (auto *includes =
        ...
        // ParseParameters() fails if a non-negated -includeconf is passed on
        // the command-line
    ...
    if (use_conf_file) {
      ...
      auto add_includes = [&](const std::string &network, size_t skip = 0) {
        ...
        if (auto *section = common::FindKey(m_settings.ro_config, network)) {
          if (auto *values = common::FindKey(*section, "includeconf")) {
            for (size_t i =
                     std::max(skip, common::SettingsSpan(*values).negated());
                 i < values->size(); ++i) {
      ...
      for (const std::string &conf_file_name : conf_file_names) {
        std::ifstream conf_file_stream{AbsPathForConfigVal(
            *this, fs::PathFromString(conf_file_name), /*net_specific=*/false)};
        if (conf_file_stream.good()) {
          if (!ReadConfigStream(conf_file_stream, conf_file_name, error,
                                ignore_invalid_keys)) {
            return false;
          }
