...
#include <vector>
  ...
  bool ReadSettings(const fs::path &path,
    ...
    if (!in.read(std::string{std::istreambuf_iterator<char>(file),
      ...
      errors.emplace_back(
          strprintf("Settings file %s does not contain valid JSON. This is "
                    "probably caused by disk corruption or a crash, "
                    "and can be fixed by removing the file, which will reset "
    ...
    if (file.fail()) {
      errors.emplace_back(
          strprintf("Failed reading settings file %s", fs::PathToString(path)));
      return false;
    }
    ...
    if (!in.isObject()) {
      errors.emplace_back(
          strprintf("Found non-object value %s in settings file %s", in.write(),
                    fs::PathToString(path)));
      return false;
    }
    ...
    for (size_t i = 0; i < in_keys.size(); ++i) {
      ...
      if (!inserted.second) {
        errors.emplace_back(
            strprintf("Found duplicate key %s in settings file %s", in_keys[i],
                      fs::PathToString(path)));
        values.clear();
        break;
      }
  ...
  bool WriteSettings(const fs::path &path,
    ...
    out.pushKV(
        ...
        strprintf("This file is automatically generated and updated by %s. "
                  "Please do not edit this file while the node "
  ...
  SettingsValue GetSetting(const Settings &settings, const std::string &section,
    ...
    MergeSettings(
        settings, section, name, [&](SettingsSpan span, Source source) {
          ...
          // value in the default section is applied to network specific
          // options, even though normal non-negated values there would be
          // ignored.
  ...
  std::vector<SettingsValue> GetSettingsList(
      const Settings &settings, const std::string &section,
      const std::string &name, bool ignore_default_section_config) {
    std::vector<SettingsValue> result;
    bool done = false; // Done merging any more settings sources.
    bool prev_negated_empty = false;
    MergeSettings(settings, section, name,
                  [&](SettingsSpan span, Source source) {
                    // Weird behavior preserved for backwards compatibility:
                    // Apply config file settings even if negated on command
                    // line. Negating a setting on command line will ignore
                    // earlier settings on the command line and ignore settings
                    // in the config file, unless the negated command line value
                    // is followed by non-negated value, in which case config
                    // file settings will be brought back from the dead (but
                    // earlier command line settings will still be ignored).
                    ...
                    // Add new settings to the result if isn't already complete,
                    // or if the values are zombies.
                    ...
                    // Update the negated and empty state used for the zombie
                    // values check.
  ...
  size_t SettingsSpan::negated() const {
    for (size_t i = size; i > 0; --i) {
      if (data[i - 1].isFalse())
        return i; // Return number of negated values (position of last false
                  // value)
    }
    return 0;
  }
