...
", fs::PathToString(path_tmp), fs::PathToString(path))}, errors);
  ...
  common::SettingsValue ArgsManager::GetPersistentSetting(
  ...
  std::optional<std::string> ArgsManager::GetArg(const std::string &strArg)
      const {
    const common::SettingsValue value = GetSetting(strArg);
    return SettingToString(value);
  }

  std::optional<std::string> SettingToString(
  ...
  int64_t ArgsManager::GetIntArg(const std::string &strArg, int64_t nDefault)
      const {
    return GetIntArg(strArg).value_or(nDefault);
  }

  std::optional<int64_t> ArgsManager::GetIntArg(const std::string &strArg)
      const {
    const common::SettingsValue value = GetSetting(strArg);
  ...
  void SetupHelpOptions(ArgsManager & args) {
    args.AddArg("-?", "Print this help message and exit",
...
std::variant<ChainType, std::string> ArgsManager::GetChainArg() const {
  ...
  if ((int)chain_arg.has_value() + (int)fRegTest + (int)fSigNet +
          (int)fTestNet >
      1) {
    throw std::runtime_error("Invalid combination of -regtest, -signet, "
                             "-testnet and -chain. Can use at most one.");
  }
  if (chain_arg) {
...
std::vector<common::SettingsValue> ArgsManager::GetSettingsList(
...
WinCmdLineArgs::~WinCmdLineArgs() { delete[] argv; }
