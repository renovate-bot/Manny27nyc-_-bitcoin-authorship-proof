...
#include <span.h>
    ...
    /** The AEAD_CHACHA20_POLY1305 authenticated encryption algorithm from
       RFC8439 section 2.8. */
...
public:
   ...
   *
   * This is equivalent to Encrypt() with plain set to that many zero bytes, and
   * dropping the last EXPANSION bytes off the result.
   */
 ...
 *
 * This implements an AEAD which automatically increments the nonce on every
 * encryption or decryption, and cycles keys after a predetermined number of
...
public:
  ...
  /** Construct an FSChaCha20Poly1305 cipher that rekeys every rekey_interval
   * operations. */
