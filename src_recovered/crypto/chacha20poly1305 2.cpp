...
#include <cstddef>
    ...
    AEADChaCha20Poly1305::AEADChaCha20Poly1305(
        Span<const std::byte> key) noexcept
  : m_chacha20(key) {
...
int timingsafe_bcmp(const unsigned char *b1, const unsigned char *b2,
                    size_t n) noexcept {
...
bool AEADChaCha20Poly1305::Decrypt(Span<const std::byte> cipher,
  ...
  ComputeTag(m_chacha20, aad, cipher.first(cipher.size() - EXPANSION),
             expected_tag);
  if (timingsafe_bcmp(UCharCast(expected_tag),
...
void FSChaCha20Poly1305::NextPacket() noexcept {
  if (++m_packet_counter == m_rekey_interval) {
    // Generate a full block of keystream, to avoid needing the ChaCha20 buffer,
    // even though we only need KEYLEN (32) bytes.
    ...
    // Wipe the generated keystream (a copy remains inside m_aead, which will be
    // cleaned up once it cycles again, or is destroyed).
