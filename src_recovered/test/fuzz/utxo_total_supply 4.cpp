...
#include <validation.h>
  ...
  const auto StoreLastTxo = [&]() {
    ...
    if (current_block->vtx.size() == 1 &&
        tx.vout.at(i).scriptPubKey[0] == OP_RETURN) {
      // also store coinbase
  ...
  const auto AppendRandomTxo = [&](CMutableTransaction &tx) {
    const auto &txo =
        txos.at(fuzzed_data_provider.ConsumeIntegralInRange<size_t>(
            0, txos.size() - 1));
    tx.vin.emplace_back(txo.first);
    tx.vout.emplace_back(txo.second.nValue,
                         txo.second.scriptPubKey); // "Forward" coin with no fee
  };
  const auto UpdateUtxoStats = [&]() {
    LOCK(chainman.GetMutex());
    chainman.ActiveChainstate().ForceFlushStateToDisk();
    utxo_stats = std::move(*Assert(kernel::ComputeUTXOStats(
        kernel::CoinStatsHashType::NONE, &chainman.ActiveChainstate().CoinsDB(),
        chainman.m_blockman, {})));
  ...
  // Assuming that the fuzzer will mine relatively short chains (less than 200
  // blocks), we want the duplicate coinbase to be not too high. Up to 300 seems
  // reasonable.
  int64_t duplicate_coinbase_height =
      fuzzed_data_provider.ConsumeIntegralInRange(0, 300);
  // Always pad with OP_0 at the end to avoid bad-cb-length error
  const CScript duplicate_coinbase_script =
      CScript() << duplicate_coinbase_height << OP_0;
    ...
    CallOneOf(
        ...
        [&] {
          ...
          if (was_valid) {
            ...
            circulation +=
                GetBlockSubsidy(ActiveHeight(), Params().GetConsensus());
          }
