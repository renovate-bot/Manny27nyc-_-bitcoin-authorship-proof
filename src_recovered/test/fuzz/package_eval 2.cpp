...
struct OutpointsUpdater final : public CValidationInterface {
  ...
  explicit OutpointsUpdater(std::set<COutPoint> &r) : m_mempool_outpoints{r} {}
...
struct TransactionsDelta final : public CValidationInterface {
  ...
  explicit TransactionsDelta(std::set<CTransactionRef> &a) : m_added{a} {}
...
void MockTime(FuzzedDataProvider &fuzzed_data_provider,
              const Chainstate &chainstate) {
  const auto time = ConsumeTime(
      fuzzed_data_provider, chainstate.m_chain.Tip()->GetMedianTimePast() + 1,
      std::numeric_limits<decltype(chainstate.m_chain.Tip()->nTime)>::max());
  SetMockTime(time);
}

std::unique_ptr<CTxMemPool>
MakeMempool(FuzzedDataProvider &fuzzed_data_provider, const NodeContext &node) {
  // Take the default options for tests...
  CTxMemPool::Options mempool_opts{MemPoolOptionsForTest(node)};

  // ...override specific options for this specific fuzz suite
  mempool_opts.limits.ancestor_count =
      fuzzed_data_provider.ConsumeIntegralInRange<unsigned>(0, 50);
  mempool_opts.limits.ancestor_size_vbytes =
      fuzzed_data_provider.ConsumeIntegralInRange<unsigned>(0, 202) * 1'000;
  mempool_opts.limits.descendant_count =
      fuzzed_data_provider.ConsumeIntegralInRange<unsigned>(0, 50);
  mempool_opts.limits.descendant_size_vbytes =
      fuzzed_data_provider.ConsumeIntegralInRange<unsigned>(0, 202) * 1'000;
  mempool_opts.max_size_bytes =
      fuzzed_data_provider.ConsumeIntegralInRange<unsigned>(0, 200) * 1'000'000;
  mempool_opts.expiry = std::chrono::hours{
      fuzzed_data_provider.ConsumeIntegralInRange<unsigned>(0, 999)};
  // Only interested in 2 cases: sigop cost 0 or when single legacy sigop cost
  // is >> 1KvB
  nBytesPerSigOp =
      fuzzed_data_provider.ConsumeIntegralInRange<unsigned>(0, 1) * 10'000;
  ...
  Assert(error.empty() ||
         error.original.starts_with("-maxmempool must be at least "));
  return mempool;
}
...
FUZZ_TARGET(tx_package_eval, .init = initialize_tx_pool) {
  ...
  auto &chainstate{
      static_cast<DummyChainState &>(node.chainman->ActiveChainstate())};
  ...
  auto outpoints_updater =
      std::make_shared<OutpointsUpdater>(mempool_outpoints);
  ...
  LIMITED_WHILE(fuzzed_data_provider.ConsumeBool(), 300) {
    ...
    const auto num_txs =
        (size_t)fuzzed_data_provider.ConsumeIntegralInRange<int>(1, 26);
    std::set<COutPoint> package_outpoints;
    while (txs.size() < num_txs) {

      // Last transaction in a package needs to be a child of parents to get
      // further in validation so the last transaction to be generated(in a >1
      // package) must spend all package-made outputs Note that this test
      // currently only spends package outputs in last transaction.
      ...
      const CTransactionRef tx = [&] {
        ...
        const auto num_in =
            last_tx ? package_outpoints.size()
                    : fuzzed_data_provider.ConsumeIntegralInRange<int>(
                          1, mempool_outpoints.size());
        auto num_out = fuzzed_data_provider.ConsumeIntegralInRange<int>(
            1, mempool_outpoints.size() * 2);
        ...
        for (size_t i = 0; i < num_in; ++i) {
          ...
          std::advance(pop, fuzzed_data_provider.ConsumeIntegralInRange<size_t>(
                                0, outpoints.size() - 1));
          ...
          const auto script_wit_stack = fuzzed_data_provider.ConsumeBool()
        ...
        // Make a p2pk output to make sigops adjusted vsize to violate v3,
        ...
        const auto amount_fee =
            fuzzed_data_provider.ConsumeIntegralInRange<CAmount>(0, amount_in);
        ...
        if (!last_tx) {
          for (const auto &in : tx->vin) {
            ...
            Assert(in == CTxIn() || outpoints.insert(in.prevout).second ||
                   dup_input);
          }
    ...
    if (fuzzed_data_provider.ConsumeBool()) {
      ...
      const auto delta = fuzzed_data_provider.ConsumeIntegralInRange<CAmount>(
          -50 * COIN, +50 * COIN);
    ...
    // When there are multiple transactions in the package, we call
    // ProcessNewPackage(txs, test_accept=false) and
    // AcceptToMemoryPool(txs.back(), test_accept=true). When there is only 1
    // transaction, we might flip it (the package is a test accept and ATMP is a
    // submission).
    ...
    if (fuzzed_data_provider.ConsumeBool()) {
      client_maxfeerate = CFeeRate(
          fuzzed_data_provider.ConsumeIntegralInRange<CAmount>(-1, 50 * COIN),
          100);
    }

    const auto result_package = WITH_LOCK(
        ::cs_main, return ProcessNewPackage(chainstate, tx_pool, txs,
                                            /*test_accept=*/single_submit,
                                            client_maxfeerate));

    // Always set bypass_limits to false because it is not supported in
    // ProcessNewPackage and can be a source of divergence.
    const auto res = WITH_LOCK(
        ::cs_main, return AcceptToMemoryPool(chainstate, txs.back(), GetTime(),
                                             /*bypass_limits=*/false,
                                             /*test_accept=*/!single_submit));
    const bool passed =
        res.m_result_type == MempoolAcceptResult::ResultType::VALID;

    node.validation_signals->SyncWithValidationInterfaceQueue();
    node.validation_signals->UnregisterSharedValidationInterface(txr);

    // There is only 1 transaction in the package. We did a test-package-accept
    // and a ATMP
    if (single_submit) {
    ...
    } else if (result_package.m_state.GetResult() !=
               PackageValidationResult::PCKG_POLICY) {
      // We don't know anything about the validity since transactions were
      // randomly generated, so just use result_package.m_state here. This makes
      // the expect_valid check meaningless, but we can still verify that the
      // contents of m_tx_results are consistent with m_state.
      const bool expect_valid{result_package.m_state.IsValid()};
      Assert(!CheckPackageMempoolAcceptResult(txs, result_package, expect_valid,
                                              nullptr));
    } else {
      // This is empty if it fails early checks, or "full" if transactions are
      // looked at deeper
      Assert(result_package.m_tx_results.size() == txs.size() ||
             result_package.m_tx_results.empty());
    }

    CheckMempoolV3Invariants(tx_pool);
  }

  node.validation_signals->UnregisterSharedValidationInterface(
      outpoints_updater);

  WITH_LOCK(::cs_main, tx_pool.check(chainstate.CoinsTip(),
