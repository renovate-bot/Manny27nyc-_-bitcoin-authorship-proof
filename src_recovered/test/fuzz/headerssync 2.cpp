...
void MakeHeadersContinuous(const CBlockHeader &genesis_header,
  ...
  const CBlockHeader *prev_header{all_headers.empty() ? &genesis_header
                                                      : &all_headers.back()};
...
public:
  FuzzedHeadersSyncState(const unsigned commit_offset,
      ...
      : HeadersSyncState(/*id=*/0, Params().GetConsensus(), chain_start,
...
FUZZ_TARGET(headers_sync_state, .init = initialize_headers_sync_state_fuzz) {
  ...
  if (mock_time < start_index.GetMedianTimePast())
    return;
  ...
  FuzzedHeadersSyncState headers_sync(
      /*commit_offset=*/fuzzed_data_provider.ConsumeIntegralInRange<unsigned>(
          1, 1024),
  ...
  while (requested_more) {
    ...
    if (presync || fuzzed_data_provider.ConsumeBool()) {
      auto deser_headers = ConsumeDeserializable<std::vector<CBlockHeader>>(
          fuzzed_data_provider);
      if (!deser_headers || deser_headers->empty())
        return;
    ...
    } else if (auto num_headers_left{
                   std::distance(redownloaded_it, all_headers.cend())};
               num_headers_left > 0) {
      // Consume some headers from the redownload buffer (At least one
      // header is consumed).
      auto begin_it{redownloaded_it};
      std::advance(redownloaded_it,
    ...
    if (headers.empty())
      return;
    auto result = headers_sync.ProcessNextHeaders(
        headers, fuzzed_data_provider.ConsumeBool());
    requested_more = result.request_more;

    if (result.request_more) {
      if (presync) {
        all_headers.insert(all_headers.cend(), headers.cbegin(),
                           headers.cend());
