...
#include <vector>
  ...
  class PoolResourceFuzzer {
    ...
    struct Entry {
      ...
      Entry(Span<std::byte> s, size_t a, uint64_t se)
          : span(s), alignment(a), seed(se) {}
    };
  ...
  public:
    PoolResourceFuzzer(FuzzedDataProvider &provider)
        : m_provider{provider},
          m_test_resource{provider.ConsumeIntegralInRange<size_t>(
              MAX_BLOCK_SIZE_BYTES, 262144)} {}
    ...
    void Allocate(size_t size, size_t alignment) {
      ...
      assert((size & (alignment - 1)) ==
             0); // Size must be a multiple of alignment.

      auto span = Span(
          static_cast<std::byte *>(m_test_resource.Allocate(size, alignment)),
          size);
    ...
    void Allocate() {
      if (m_total_allocated > 0x1000000)
        return;
      size_t alignment_bits = m_provider.ConsumeIntegralInRange<size_t>(0, 7);
      size_t alignment = 1 << alignment_bits;
      size_t size_bits =
          m_provider.ConsumeIntegralInRange<size_t>(0, 16 - alignment_bits);
      size_t size = m_provider.ConsumeIntegralInRange<size_t>(
                        1U << size_bits, (1U << (size_bits + 1)) - 1U)
                    << alignment_bits;
    ...
    void Deallocate(const Entry &entry) {
      ...
      m_test_resource.Deallocate(entry.span.data(), entry.span.size(),
                                 entry.alignment);
    }
    ...
    void Deallocate() {
      ...
      size_t idx =
          m_provider.ConsumeIntegralInRange<size_t>(0, m_entries.size() - 1);
    ...
    void Fuzz() {
      LIMITED_WHILE(m_provider.ConsumeBool(), 10000) {
        CallOneOf(m_provider, [&] { Allocate(); }, [&] { Deallocate(); });
      }
      Clear();
    }
  };
...
FUZZ_TARGET(pool_resource) {
  ...
  CallOneOf(
      provider, [&] { PoolResourceFuzzer<128, 1>{provider}.Fuzz(); },
