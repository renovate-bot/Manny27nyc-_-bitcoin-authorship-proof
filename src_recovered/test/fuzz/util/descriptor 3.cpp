...
#include <test/fuzz/util/descriptor.h>
  ...
  for (size_t i{0}; i < TOTAL_KEYS_GENERATED; i++) {
    ...
    // an extended one.
    if (IdIsCompPubKey(i) || IdIsUnCompPubKey(i) || IdIsXOnlyPubKey(i) ||
        IdIsConstPrivKey(i)) {
      CKey privkey;
      privkey.Set(UCharCast(key_data.begin()), UCharCast(key_data.end()),
                  !IdIsUnCompPubKey(i));
...
MockedDescriptorConverter::IdxFromHex(std::string_view hex_characters) const {
  if (hex_characters.size() != 2)
    return {};
...
MockedDescriptorConverter::GetDescriptor(std::string_view mocked_desc) const {
  // The smallest fragment would be "pk(%00)"
  if (mocked_desc.size() < 7)
    return {};
  ...
  // Replace all occurrences of '%' followed by two hex characters with the
  // corresponding key.
  for (size_t i = 0; i < mocked_desc.size();) {
    if (mocked_desc[i] == '%') {
      if (i + 3 >= mocked_desc.size())
        return {};
...
bool HasDeepDerivPath(const FuzzBufferType &buff, const int max_depth) {
  ...
  for (const auto &ch : buff) {
    if (ch == ',') {
      // A comma is always present between two key expressions, so we use that
      // as a delimiter.
      depth = 0;
    } else if (ch == '/') {
      if (++depth > max_depth)
