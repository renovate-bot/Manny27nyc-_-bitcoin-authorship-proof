...
#include <iostream>
  ...
  std::strong_ordering compare_arrays(const std::array<uint32_t, 4> &a,
  ...
  std::strong_ordering MulCompare(int64_t a1, int64_t a2, int64_t b1,
                                  int64_t b2) {
    ...
    uint64_t abs_a1 = static_cast<uint64_t>(a1),
             abs_a2 = static_cast<uint64_t>(a2);
    uint64_t abs_b1 = static_cast<uint64_t>(b1),
             abs_b2 = static_cast<uint64_t>(b2);
    // Use (~x + 1) instead of the equivalent (-x) to silence the linter; mod
    // 2^64 behavior is intentional here.
    if (a1 < 0)
      abs_a1 = ~abs_a1 + 1;
    if (a2 < 0)
      abs_a2 = ~abs_a2 + 1;
...
FUZZ_TARGET(feefrac) {
  ...
  auto cmp_fallback =
      FeeFrac::MulFallback(f1, s2) <=> FeeFrac::MulFallback(f2, s1);
