...
#include <vector>
  ...
  FUZZ_TARGET(mini_miner, .init = initialize_miner) {
    ...
    LIMITED_WHILE(!available_coins.empty(), 500) {
      ...
      const size_t num_inputs =
          fuzzed_data_provider.ConsumeIntegralInRange<size_t>(
              1, available_coins.size());
      const size_t num_outputs =
          fuzzed_data_provider.ConsumeIntegralInRange<size_t>(1, 50);
      ...
      const CAmount fee{
          ConsumeMoney(fuzzed_data_provider, /*max=*/MAX_MONEY / 100000)};
      ...
      if (fuzzed_data_provider.ConsumeBool() && !tx->vout.empty()) {
        ...
        outpoints.emplace_back(
            tx->GetHash(),
            (uint32_t)fuzzed_data_provider.ConsumeIntegralInRange<size_t>(
                0, tx->vout.size()));
      } else {
        // Add some random outpoint (will be interpreted as confirmed or not yet
        // submitted to mempool).
        auto outpoint = ConsumeDeserializable<COutPoint>(fuzzed_data_provider);
        if (outpoint.has_value() &&
            std::find(outpoints.begin(), outpoints.end(), *outpoint) ==
                outpoints.end()) {
    ...
    const CFeeRate target_feerate{
        CFeeRate{ConsumeMoney(fuzzed_data_provider, /*max=*/MAX_MONEY / 1000)}};
    ...
    // Overlapping ancestry across multiple outpoints can only reduce the total
    // bump fee.
    assert(sum_fees >= *total_bumpfee);
  }

  // Test that MiniMiner and BlockAssembler build the same block given the same
  ...
  FUZZ_TARGET(mini_miner_selection, .init = initialize_miner) {
    ...
    LIMITED_WHILE(fuzzed_data_provider.ConsumeBool(), 100) {
      ...
      const size_t num_outputs =
          fuzzed_data_provider.ConsumeIntegralInRange<size_t>(2, 5);
      for (size_t n{0}; n < num_inputs; ++n) {
      ...
      // First 2 outputs are available to spend. The rest are added to outpoints
      // to calculate bumpfees. There is no overlap between spendable coins and
      // outpoints passed to MiniMiner because the MiniMiner interprets spent
      // coins as to-be-replaced and excludes them.
      ...
      // Stop if pool reaches DEFAULT_BLOCK_MAX_WEIGHT because BlockAssembler
      // will stop when the block template reaches that, but the MiniMiner will
      // keep going.
      if (pool.GetTotalTxSize() + GetVirtualTransactionSize(*tx) >=
    ...
    for (const auto &coin : g_available_coins) {
      if (!pool.GetConflictTx(coin))
        outpoints.push_back(coin);
    }
    ...
    node::BlockAssembler miner{g_setup->m_node.chainman->ActiveChainstate(),
                               &pool, miner_options};
    node::MiniMiner mini_miner{pool, outpoints};
    assert(mini_miner.IsReadyToCalculate());

    CScript spk_placeholder = CScript() << OP_0;
    // Use BlockAssembler as oracle. BlockAssembler and MiniMiner should select
    // the same transactions, stopping once packages do not meet target_feerate.
    ...
    assert(mock_template_txids.count(blocktemplate->block.vtx[0]->GetHash()) ==
           0);
