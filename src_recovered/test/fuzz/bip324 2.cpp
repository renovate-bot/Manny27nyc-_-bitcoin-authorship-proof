...
FUZZ_TARGET(bip324_cipher_roundtrip, .init = Initialize) {
  ...
  // Initialize RNG deterministically, to generate contents and AAD. We assume
  // that there are no (potentially buggy) edge cases triggered by specific
  // values of contents/AAD, so we can avoid reading the actual data for those
  // from the fuzzer input (which would need large amounts of data).
  XoRoShiRo128PlusPlus rng(provider.ConsumeIntegral<uint64_t>());

  // Compare session IDs and garbage terminators.
  assert(initiator.GetSessionID() == responder.GetSessionID());
  assert(initiator.GetSendGarbageTerminator() ==
         responder.GetReceiveGarbageTerminator());
  assert(initiator.GetReceiveGarbageTerminator() ==
         responder.GetSendGarbageTerminator());

  LIMITED_WHILE(provider.remaining_bytes(), 1000) {
    // Mode:
    // - Bit 0: whether the ignore bit is set in message
    // - Bit 1: whether the responder (0) or initiator (1) sends
    // - Bit 2: whether this ciphertext will be corrupted (making it the last
    // sent one)
    // - Bit 3-4: controls the maximum aad length (max 4095 bytes)
    // - Bit 5-7: controls the maximum content length (max 16383 bytes, for
    // performance reasons)
    ...
    unsigned aad_length = provider.ConsumeIntegralInRange<unsigned>(
        0, (1 << aad_length_bits) - 1);
    unsigned length_bits = 2 * ((mode >> 5) & 7);
    unsigned length =
        provider.ConsumeIntegralInRange<unsigned>(0, (1 << length_bits) - 1);
    ...
    // Optionally damage 1 bit in either the ciphertext (corresponding to a
    // change in transit) or the aad (to make sure that decryption will fail if
    // the AAD mismatches).
    if (damage) {
      unsigned damage_bit = provider.ConsumeIntegralInRange<unsigned>(
          0, (ciphertext.size() + aad.size()) * 8U - 1U);
    ...
    // Decrypt length
    uint32_t dec_length =
        receiver.DecryptLength(Span{ciphertext}.first(initiator.LENGTH_LEN));
    if (!damage) {
      assert(dec_length == length);
    } else {
      // For performance reasons, don't try to decode if length got increased
      // too much.
      if (dec_length > 16384 + length)
        break;
    ...
    bool ok = receiver.Decrypt(Span{ciphertext}.subspan(initiator.LENGTH_LEN),
                               aad, dec_ignore, decrypt);
    // Decryption *must* fail if the packet was damaged, and succeed if it
    // wasn't.
    assert(!ok == damage);
    if (!ok)
      break;
