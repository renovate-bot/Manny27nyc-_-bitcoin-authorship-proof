...
#include <vector>
  ...
  struct PrecomputedData {
    ...
    PrecomputedData() {
      static const uint8_t PREFIX_O[1] = {
          'o'}; /** Hash prefix for outpoint hashes. */
      static const uint8_t PREFIX_S[1] = {
          's'}; /** Hash prefix for coins scriptPubKeys. */
      static const uint8_t PREFIX_M[1] = {
          'm'}; /** Hash prefix for coins nValue/fCoinBase. */
      ...
      for (uint32_t i = 0; i < NUM_COINS; ++i) {
        const uint8_t ser[4] = {uint8_t(i), uint8_t(i >> 8), uint8_t(i >> 16),
                                uint8_t(i >> 24)};
        uint256 hash;
        CSHA256()
            .Write(PREFIX_S, 1)
            .Write(ser, sizeof(ser))
            .Finalize(hash.begin());
        /* Convert hash to scriptPubkeys (of different lengths, so SanityCheck's
        ...
        case 0: /* P2PKH */
          ...
          std::copy(hash.begin(), hash.begin() + 20,
                    coins[i].out.scriptPubKey.begin() + 3);
        ...
        case 1: /* P2SH */
          ...
          std::copy(hash.begin(), hash.begin() + 20,
                    coins[i].out.scriptPubKey.begin() + 2);
        ...
        case 3: /* P2WSH */
          ...
          std::copy(hash.begin(), hash.begin() + 32,
                    coins[i].out.scriptPubKey.begin() + 2);
          break;
        case 4: /* P2TR */
          coins[i].out.scriptPubKey.resize(34);
        ...
        coins[i].nHeight =
            0; /* Real nHeight used in simulation is set dynamically. */
      }
    }
  };
  ...
  struct CacheEntry {
    ...
    /* Index in the coins array this entry corresponds to (only if entrytype ==
     * UNSPENT). */
   ...
   * index is 3 (mod 5) always have GetCoin() succeed (but returning an
   * IsSpent() coin unless a UTXO exists). Coins whose output index is 4 (mod 5)
   * have GetCoin() always succeed after being spent. This exercises code paths
   * with spent, non-DIRTY cache entries.
   */
    ...
    bool BatchWrite(CCoinsMap &data, const uint256 &, bool erase) final {
      for (auto it = data.begin(); it != data.end();
        ...
        if (it->second.flags & CCoinsCacheEntry::DIRTY) {
          ...
          /* For non-dirty entries being written, compare them with what we
           * have. */
    ...
    CallOneOf(
        ...
        [&]() { // GetCoin
          uint32_t outpointidx =
              provider.ConsumeIntegralInRange<uint32_t>(0, NUM_OUTPOINTS - 1);
          // Look up in simulation data.
          auto sim = lookup(outpointidx);
          // Look up in real caches.
          Coin realcoin;
          auto real =
              caches.back()->GetCoin(data.outpoints[outpointidx], realcoin);
          // Compare results.
          if (!sim.has_value()) {
        ...
        [&]() { // HaveCoinInCache
          uint32_t outpointidx =
              provider.ConsumeIntegralInRange<uint32_t>(0, NUM_OUTPOINTS - 1);
          // Invoke on real cache (there is no equivalent in simulation, so
          // nothing to compare result with).
        ...
        [&]() { // AddCoin (only possible_overwrite if necessary)
          ...
          caches.back()->AddCoin(data.outpoints[outpointidx], std::move(coin),
                                 sim.has_value());
        ...
        [&]() { // Add a cache level (if not already at the max).
          if (caches.size() != MAX_CACHES) {
            ...
            caches.emplace_back(
                new CCoinsViewCache(&*caches.back(), /*deterministic=*/true));
            // Apply to simulation data.
            sim_caches[caches.size()].Wipe();
          }
        },
        ...
        [&]() { // Remove a cache level.
          // Apply to real caches (this reduces caches.size(), implicitly doing
          // the same on the simulation data).
        ...
        [&]() { // Sync.
          // Apply to simulation data (note that in our simulation, syncing and
          // flushing is the same thing).
          flush();
          // Apply to real caches.
          caches.back()->Sync();
        },
        ...
        [&]() { // Change height
          current_height =
              provider.ConsumeIntegralInRange<uint32_t>(1, current_height - 1);
        });
  }
