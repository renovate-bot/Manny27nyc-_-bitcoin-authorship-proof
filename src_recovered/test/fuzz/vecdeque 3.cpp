...
#include <stdint.h>
   ...
   *
   * T must be constructible from a uint64_t seed, comparable to other T,
  ...
  void TestType(Span<const uint8_t> buffer, uint64_t rng_tweak) {
    ...
    LIMITED_WHILE(provider.remaining_bytes(), MAX_OPERATIONS) {
      ...
      unsigned idx =
          real.empty()
              ? 0
              : provider.ConsumeIntegralInRange<unsigned>(0, real.size() - 1);
      const size_t num_buffers = sim.size();
      // Pick one operation based on value of command. Not all operations are
      // always applicable. Loop through the applicable ones until command
      // reaches 0 (which avoids the need to compute the number of applicable
      ...
      const bool existing_buffer_non_full{non_empty &&
                                          sim[idx].size() < MAX_BUFFER_SIZE};
      ...
      while (true) {
        ...
        if (non_empty && command-- == 0) {
          ...
          size_t new_size =
              provider.ConsumeIntegralInRange<size_t>(0, MAX_BUFFER_SIZE);
        ...
        if (non_empty && command-- == 0) {
          /* Self-move assign. */
          // Do not use std::move(real[idx]) here: -Wself-move correctly warns
          // about that.
        ...
        if (existing_buffer_non_empty && command-- == 0) {
          ...
          tmp = T(rng());
          size_t pos =
              provider.ConsumeIntegralInRange<size_t>(0, sim[idx].size() - 1);
          size_t old_size = real[idx].size();
    ...
    for (unsigned i = 0; i < sim.size(); ++i) {
      ...
      // Clear out the buffers so we can check below that no objects exist
      // anymore.
      sim[i].clear();
      real[i].clear();
    }
  ...
  template <size_t Size> class TrackedObj {
    static_assert(Size > 0);

    /* Data type for map that actually stores the object data.
     *
     * The key is a pointer to the TrackedObj, the value is the uint64_t it was
     * initialized with. Default-constructed and moved-from objects hold an
     * std::nullopt.
     */
    using track_map_type =
        std::map<const TrackedObj<Size> *, std::optional<uint64_t>>;
  ...
  private:
     ...
     *
     * This is an array of size Size, all holding the same value, to give the
     * object configurable size. The value is g_tracker.end() if this object is
  ...
  public:
    ...
    friend std::strong_ordering operator<=>(const TrackedObj &a,
                                            const TrackedObj &b) {
    ...
    static void CheckNoneExist() { assert(g_tracker.empty()); }
  };
