...
#include <boost/test/unit_test.hpp>
  ...
  // Invalid version.
  BOOST_CHECK_EQUAL(tracker.RegisterPeer(/*peer_id=*/0,
                                         /*is_peer_inbound=*/true,
  ...
  BOOST_REQUIRE_EQUAL(tracker.RegisterPeer(0, true, 1, salt),
  ...
  BOOST_REQUIRE(tracker.RegisterPeer(1, false, 1, salt) ==
                ReconciliationRegisterResult::SUCCESS);
  ...
  BOOST_REQUIRE(tracker.RegisterPeer(2, true, 2, salt) ==
                ReconciliationRegisterResult::SUCCESS);
  ...
  BOOST_REQUIRE_EQUAL(tracker.RegisterPeer(100, true, 1, salt),
...
BOOST_AUTO_TEST_CASE(ForgetPeerTest) {
  ...
  // Removing peer after pre-registring works and does not let to register the
  // peer.
  tracker.PreRegisterPeer(peer_id0);
  tracker.ForgetPeer(peer_id0);
  BOOST_CHECK_EQUAL(tracker.RegisterPeer(peer_id0, true, 1, 1),
                    ReconciliationRegisterResult::NOT_FOUND);

  // Removing peer after it is registered works.
  tracker.PreRegisterPeer(peer_id0);
  BOOST_REQUIRE(!tracker.IsPeerRegistered(peer_id0));
  BOOST_REQUIRE_EQUAL(tracker.RegisterPeer(peer_id0, true, 1, 1),
                      ReconciliationRegisterResult::SUCCESS);
