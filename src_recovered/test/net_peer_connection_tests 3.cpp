...
/** Create a peer and connect to it. If the optional `address` (IP/CJDNS only)
 * isn't passed, a random address is created. */
static void AddPeer(NodeId &id, std::vector<CNode *> &nodes,
                    PeerManager &peerman, ConnmanTestMsg &connman,
                    ConnectionType conn_type, bool onion_peer = false,
                    std::optional<std::string> address = std::nullopt) {
  CAddress addr{};

  if (address.has_value()) {
    addr = CAddress{MaybeFlipIPv6toCJDNS(LookupNumeric(
                        address.value(), Params().GetDefaultPort())),
                    NODE_NONE};
  } else if (onion_peer) {
...
BOOST_AUTO_TEST_CASE(test_addnode_getaddednodeinfo_and_connection_detection) {
  auto connman = std::make_unique<ConnmanTestMsg>(
      0x1337, 0x1337, *m_node.addrman, *m_node.netgroupman, Params());
  ...
  // Connect a localhost peer.
  {
    ...
    AddPeer(id, nodes, *peerman, *connman, ConnectionType::MANUAL,
            /*onion_peer=*/false, /*address=*/"127.0.0.1");
    BOOST_REQUIRE(nodes.back() != nullptr);
  }

  // Call ConnectNode(), which is also called by RPC addnode onetry, for a
  // localhost address that resolves to multiple IPs, including that of the
  // connected peer. The connection attempt should consistently fail due to the
  // check in ConnectNode().
  for (int i = 0; i < 10; ++i) {
    ASSERT_DEBUG_LOG("Not opening a connection to localhost, already connected "
                     "to 127.0.0.1:8333");
    BOOST_CHECK(!connman->ConnectNodePublic(*peerman, "localhost",
                                            ConnectionType::MANUAL));
  }

  // Add 3 more peer connections.
  AddPeer(id, nodes, *peerman, *connman, ConnectionType::OUTBOUND_FULL_RELAY);
  AddPeer(id, nodes, *peerman, *connman, ConnectionType::BLOCK_RELAY,
          /*onion_peer=*/true);
  ...
  BOOST_CHECK_EQUAL(nodes.back()->ConnectedThroughNetwork(),
                    Network::NET_CJDNS);

  BOOST_TEST_MESSAGE("Call AddNode() for all the peers");
  for (auto node : connman->TestNodes()) {
    BOOST_CHECK(
        connman->AddNode({/*m_added_node=*/node->addr.ToStringAddrPort(),
                          /*m_use_v2transport=*/true}));
    BOOST_TEST_MESSAGE(strprintf("peer id=%s addr=%s", node->GetId(),
...
Call AddNode() with 2 addrs resolving to existing localhost addnode entry; neither should be added");
    ...
    // OpenBSD doesn't support the IPv4 shorthand notation with omitted
    // zero-bytes.
...
added node: %s", info.m_params.m_added_node));
        ...
        if (info.fConnected) {
      BOOST_TEST_MESSAGE(
          strprintf("IP address: %s", info.resolvedAddress.ToStringAddrPort()));
      BOOST_TEST_MESSAGE(
          strprintf("direction: %s", info.fInbound ? "inbound" : "outbound"));
        }
    }
