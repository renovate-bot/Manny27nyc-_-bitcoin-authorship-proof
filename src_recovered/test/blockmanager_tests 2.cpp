...
BOOST_AUTO_TEST_CASE(blockmanager_find_block_pos) {
  ...
  KernelNotifications notifications{*Assert(m_node.shutdown),
                                    m_node.exit_status};
  ...
  // blk00000.dat file the block is found at offset 8 because there is an 8 byte
  // serialization header consisting of 4 magic bytes + 4 length bytes before
  // each block in a well-formed blk file.
  ...
  // this is a check to make sure that
  // https://github.com/bitcoin/bitcoin/issues/21379 does not recur 8 bytes (for
  // serialization header) + 285 (for serialized genesis block) = 293 add
  // another 8 bytes for the second block's serialization header and we get 293
  // + 8 = 301
  FlatFilePos actual{blockman.SaveBlockToDisk(params->GenesisBlock(), 1)};
  BOOST_CHECK_EQUAL(actual.nPos, BLOCK_SERIALIZATION_HEADER_SIZE +
...
BOOST_FIXTURE_TEST_CASE(blockmanager_scan_unlink_already_pruned_files,
  ...
  WITH_LOCK(chainman->GetMutex(),
            blockman.GetBlockFileInfo(old_tip->GetBlockPos().nFile)->nSize =
                MAX_BLOCKFILE_SIZE);
...
BOOST_AUTO_TEST_CASE(blockmanager_flush_block_file) {
  ...
  // Errors are expected because block data is junk, thrown AFTER successful
  // read
  CBlock read_block;
