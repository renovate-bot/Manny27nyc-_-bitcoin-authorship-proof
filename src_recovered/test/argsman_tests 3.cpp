...
BOOST_AUTO_TEST_CASE(util_datadir) {
  // Use local args variable instead of m_args to avoid making assumptions about
  // test setup
  ArgsManager args;
            ...
            for (const std::string &section :
              ...
              for (const std::string &network :
                ...
                for (bool net_specific : {false, true}) {
                  fn(arg_actions, conf_actions, soft_set, force_set, section,
                     network, net_specific);
                }
              }
            }
          }
...
BOOST_FIXTURE_TEST_CASE(util_ArgsMerge, ArgsMergeTestingSetup) {
  ...
  if (out_file) {
    if (fclose(out_file))
      throw std::system_error(errno, std::generic_category(), "fclose failed");
    out_file = nullptr;
  }
  ...
  //   --run_test=argsman_tests/util_ArgsMerge
  //
...
BOOST_FIXTURE_TEST_CASE(util_ChainMerge, ChainMergeTestingSetup) {
  ...
  ForEachMergeSetup(
      [&](const ActionList &arg_actions, const ActionList &conf_actions) {
        TestArgsManager parser;
        LOCK(parser.cs_args);
        ...
        auto arg = [](Action action) {
          return action == ENABLE_TEST    ? "-testnet=1"
  ...
  //   --run_test=argsman_tests/util_ChainMerge
  //
  // And verify diff against previous results to make sure the changes are
  // expected.
  //
...
BOOST_AUTO_TEST_CASE(util_ReadWriteSettings) {
  ...
  args1.LockSettings([&](common::Settings &settings) {
  ...
  args2.LockSettings([&](common::Settings &settings) {
