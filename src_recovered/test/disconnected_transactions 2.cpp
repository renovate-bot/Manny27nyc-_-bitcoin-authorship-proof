...
#include <test/util/setup_common.h>
  ...
  // Use the coinbase transactions from TestChain100Setup. It doesn't matter
  // whether these transactions would realistically be in a block together, they
  // just need distinct txids and uniform size for this test to work.
  std::vector<CTransactionRef> block_vtx(m_coinbase_txns);
  BOOST_CHECK_EQUAL(block_vtx.size(), 100);

  // Roughly estimate sizes to sanity check that
  // DisconnectedBlockTransactions::DynamicMemoryUsage is within an expected
  // range.
  ...
  // Implementations may vary, but we're trying to guess the usage of data
  // structures.
  const size_t ENTRY_USAGE_ESTIMATE{
      TX_USAGE
      // list entry: 2 pointers (next pointer and prev pointer) + element itself
      + memusage::MallocUsage((2 * sizeof(void *)) +
                              sizeof(decltype(block_vtx)::value_type))
      // unordered map: 1 pointer for the hashtable + key and value
      + memusage::MallocUsage(sizeof(void *) +
                              sizeof(decltype(temp_map)::key_type) +
  ...
  // DisconnectedBlockTransactions that's just big enough for 1 transaction.
  {
    ...
    // Add just 2 (and not all 100) transactions to keep the unordered map's
    // hashtable overhead to a minimum and avoid all (instead of all but 1)
    // transactions getting evicted.
    ...
    BOOST_CHECK(disconnectpool.DynamicMemoryUsage() <=
                MAP_1 + ENTRY_USAGE_ESTIMATE);
  ...
  // DisconnectedBlockTransactions with a comfortable maximum memory usage so
  // that nothing is evicted. Record usage so we can check size limiting in the
  // next test.
  size_t usage_full{0};
  {
  ...
  // DisconnectedBlockTransactions that's just a little too small for all of the
  // transactions.
  {
