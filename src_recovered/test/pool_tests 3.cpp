...
#include <vector>
  ...
  BOOST_TEST(expected_bytes_available ==
             PoolResourceTester::AvailableMemoryFromChunk(resource));
  ...
  // alignment is too small, but the best fitting freelist is used. Nothing is
  // allocated.
  ...
  // it's possible that 0 bytes are allocated, make sure this works. In that
  // case the call is forwarded to operator new 0 bytes takes one entry from the
  // first freelist
...
// Allocates from 0 to n bytes were n > the PoolResource's data, and each should
// work
BOOST_AUTO_TEST_CASE(allocate_any_byte) {
  ...
  // now that we got all allocated, test if all still have the correct values,
  // and give everything back to the allocator
  uint8_t val = 0;
...
BOOST_AUTO_TEST_CASE(random_allocations) {
  ...
  // makes a bunch of random allocations and gives all of them back in random
  // order.
  ...
  for (size_t i = 0; i < 1000; ++i) {
    ...
    if (ptr_size_alignment.empty() || 0 != InsecureRandRange(4)) {
      ...
      std::size_t alignment = std::size_t{1}
                              << InsecureRandRange(8); // 1, 2, ..., 128
      std::size_t size = (InsecureRandRange(200) / alignment + 1) *
                         alignment; // multiple of alignment
      ...
      auto &x =
          ptr_size_alignment[InsecureRandRange(ptr_size_alignment.size())];
...
BOOST_AUTO_TEST_CASE(memusage_test) {
...

  {
    ...
    // Eventually the resource_map should have a much lower memory usage because
    // it has less malloc overhead
