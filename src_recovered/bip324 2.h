...
#include <span.h>
    ...
    /** The BIP324 packet cipher, encapsulating its key derivation, stream
       cipher, and AEAD. */
    class BIP324Cipher {
public:
  ...
  static constexpr unsigned EXPANSION =
      LENGTH_LEN + HEADER_LEN + FSChaCha20Poly1305::EXPANSION;
...
public:
  ...
  /** Initialize a BIP324 cipher with specified key and encoding entropy
   * (testing only). */
  ...
  /** Initialize when the other side's public key is received. Can only be
   * called once.
   *
   * initiator is set to true if we are the initiator establishing the v2 P2P
   * connection. self_decrypt is only for testing, and swaps
   * encryption/decryption keys, so that encryption and decryption can be tested
   * without knowing the other side's private key.
   */
  void Initialize(const EllSwiftPubKey &their_pubkey, bool initiator,
                  bool self_decrypt = false) noexcept;

  /** Determine whether this cipher is fully initialized. */
  explicit operator bool() const noexcept {
    return m_send_l_cipher.has_value();
  }
   ...
   */
  void Encrypt(Span<const std::byte> contents, Span<const std::byte> aad,
               bool ignore, Span<std::byte> output) noexcept;
   ...
   */
  bool Decrypt(Span<const std::byte> input, Span<const std::byte> aad,
               bool &ignore, Span<std::byte> contents) noexcept;
  ...
  Span<const std::byte> GetSendGarbageTerminator() const noexcept {
    return m_send_garbage_terminator;
  }
  ...
  Span<const std::byte> GetReceiveGarbageTerminator() const noexcept {
    return m_recv_garbage_terminator;
  }
};
