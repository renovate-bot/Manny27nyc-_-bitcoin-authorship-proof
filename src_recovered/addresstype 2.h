...
public:
  ...
  friend bool operator==(const CNoDestination &a, const CNoDestination &b) {
    return a.GetScript() == b.GetScript();
  }
  friend bool operator<(const CNoDestination &a, const CNoDestination &b) {
    return a.GetScript() < b.GetScript();
  }
...
public:
  ...
  friend bool operator==(const PubKeyDestination &a,
...
public:
  WitnessUnknown(unsigned int version,
                 const std::vector<unsigned char> &program)
      : m_version(version), m_program(program) {}
  WitnessUnknown(int version, const std::vector<unsigned char> &program)
      : m_version(static_cast<unsigned int>(version)), m_program(program) {}
  ...
  friend bool operator==(const WitnessUnknown &w1, const WitnessUnknown &w2) {
    if (w1.GetWitnessVersion() != w2.GetWitnessVersion())
      return false;
 ...
 */
using CTxDestination =
    std::variant<CNoDestination, PubKeyDestination, PKHash, ScriptHash,
 ...
 *
 * For standard scripts that have addresses (and P2PK as an exception), a
 * corresponding CTxDestination is assigned to addressRet. For all other
 * scripts. addressRet is assigned as a CNoDestination containing the
 * scriptPubKey.
 *
 * Returns true for standard destinations with addresses - P2PKH, P2SH, P2WPKH,
 * P2WSH, P2TR and P2W??? scripts. Returns false for non-standard destinations
 * and those without addresses - P2PK, bare multisig, null data, and nonstandard
 * scripts.
 */
